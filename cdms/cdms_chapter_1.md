...
Masking is covered in [Section 2.9](cdms_chapter_2.html#2.9). See also the documentation of the Python Numeric and MA modules, on which `cdms.MV` is based, at [http://numpy.sourceforge.net](http://numpy.sourceforge.net/).
...
![curvilinear grid]({{site.baseurl}}/media/images/curvilinear_grid.jpg)
...
![generic grid]({{site.baseurl}}/media/images/generic_grid.jpg)
...
To interpolate between any lat-lon grid types, the SCRIP regridder may be used. The SCRIP package was developed at [Los Alamos National Laboratory](http://oceans11.lanl.gov/drupal/Models/OtherSoftware). SCRIP is written in Fortran 90, and must be built and installed separately from the CDAT/CDMS installation.
...
Regridding is discussed in [Chapter 4](cdms_chapter_4.md).
...
The plot routine has a number of options for producing different types of plots, such as isofill and x-y plots. See [ Chapter 5 ](cdms_chapter_5.html) for details.
...
Databases are discussed further in [Section 2.7](cdms_chapter_2.html#2.7).

#### [Table of Contents](cdms.html) \| [Next Chapter](cdms_chapter_2.html)
